...
final class ConnectionManager: NSObject {
  ...
  private var discoveryDelegates = [WeakHolder]()
  private var sessionDelegates = [WeakHolder]()
  ...
  func addDiscoveryObserver(_ observer: ConnectionManagerDiscoveryDelegate) {
    discoveryDelegates.append(WeakHolder(observer))
  }

  func addSessionObserver(_ observer: ConnectionManagerSessionDelegate) {
    sessionDelegates.append(WeakHolder(observer))
  }
...
extension ConnectionManager: MCSessionDelegate {
    ...
    if let str = String(data: data, encoding: .utf8) {
      if str.first == "&" {
        if str == SendFlags.Voice.start {
          isGettingVoice = true
          talkBlocked(withReason: .receiving)
          return
          ...
          if components[0] == SendFlags.Location.flag {
            ...
            {
              updatePeerLocation(with: CLLocation(latitude: lat, longitude: lon))
            }
          }
        }
      } else {
  ...
  func session(_ session: MCSession, peer peerID: MCPeerID, didChange state: MCSessionState) {
    switch state {
    case .notConnected:
      disconnectedFromPeer(.init(mcPeer: peerID))
    case .connecting:
      break
    case .connected:
      connectedToPeer(.init(mcPeer: peerID))
    @unknown default:
      break
    }
  }
...
extension ConnectionManager: MCNearbyServiceBrowserDelegate {
  func browser(_ browser: MCNearbyServiceBrowser, foundPeer peerID: MCPeerID, withDiscoveryInfo info: [String: String]?) {
    peerFound(.init(mcPeer: peerID))
  }

  func browser(_ browser: MCNearbyServiceBrowser, lostPeer peerID: MCPeerID) {
    peerLost(.init(mcPeer: peerID))
  }
}

extension ConnectionManager: ConnectionManagerSessionDelegate {
  func talkBlocked(withReason reason: TalkBlockReason) {
    sessionDelegates.forEach { delegate in
      (delegate.object as? ConnectionManagerSessionDelegate)?.talkBlocked(withReason: reason)
    }
  }

  func talkUnblocked() {
    sessionDelegates.forEach { delegate in
      (delegate.object as? ConnectionManagerSessionDelegate)?.talkUnblocked()
    }
  }

  func updatePeerLocation(with location: CLLocation) {
    sessionDelegates.forEach { delegate in
      (delegate.object as? ConnectionManagerSessionDelegate)?.updatePeerLocation(with: location)
    }
  }
}

extension ConnectionManager: ConnectionManagerDiscoveryDelegate {
  func peerFound(_ peer: PeerModel) {
    discoveryDelegates.forEach { delegate in
      (delegate.object as? ConnectionManagerDiscoveryDelegate)?.peerFound(peer)
    }
  }

  func peerLost(_ peer: PeerModel) {
    discoveryDelegates.forEach { delegate in
      (delegate.object as? ConnectionManagerDiscoveryDelegate)?.peerLost(peer)
    }
  }

  func connectedToPeer(_ peer: PeerModel) {
    discoveryDelegates.forEach { delegate in
      (delegate.object as? ConnectionManagerDiscoveryDelegate)?.connectedToPeer(peer)
    }
  }

  func disconnectedFromPeer(_ peer: PeerModel) {
    discoveryDelegates.forEach { delegate in
      (delegate.object as? ConnectionManagerDiscoveryDelegate)?.disconnectedFromPeer(peer)
    }
  }
}
